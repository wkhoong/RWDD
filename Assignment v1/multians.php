...
$correct_answers = explode(',', $questionData['correct_answer']); // Split the correct answers into an array

// Store the correct answers as a comma-separated string
$correct_answers_str = implode(',', $correct_answers);

// Prepare and bind
$stmt = $conn->prepare("INSERT INTO quiz_questions (subject, question, option_a, option_b, option_c, option_d, correct_answer) VALUES (?, ?, ?, ?, ?, ?, ?)");
$stmt->bind_param("sssssss", $subject, $question, $option_a, $option_b, $option_c, $option_d, $correct_answers_str);
...
<html>
<head>
    <title>Create Multiple Answer Questions</title>
...
.button.active {
    ...
    color: #ffffff; /* Active button text color */
        ...
        .answer-options input.answer-input {
            background-color: #3a7d5e;
            border: none;
            border-radius: 5px;
            padding: 20px;
        ...
        .answer-options button:nth-child(2) {
            background-color: #4a8d6e;
        }
        .button-container-2 {
...
<body>
    ...
    <button class="back-button" id="back-button">Back</button>
        ...
        <div class="container">
            <div class="button-container">
                ...
                <button class="button active">Multiple correct answers</button>
    ...
    <div class="answer-options">
        <label>
            <input type="checkbox" name="correct_answer[]" value="A"> Option A
        </label>
        <label>
            <input type="checkbox" name="correct_answer[]" value="B"> Option B
        </label>
        <label>
            <input type="checkbox" name="correct_answer[]" value="C"> Option C
        </label>
        <label>
            <input type="checkbox" name="correct_answer[]" value="D"> Option D
        </label>
    </div>
    ...
    document.querySelector('.next-button').addEventListener('click', function() {
        ...
        const correctAnswerInputs = document.querySelectorAll('input[name="correct_answer[]"]:checked');
        const correctAnswers = Array.from(correctAnswerInputs).map(input => input.value);

        // Store the question and answer options
        if (questionText && answerOptions.every(option => option) && correctAnswers.length > 0) {
            // Store the current question at the current index
            questions[currentQuestionIndex] = {
                question: questionText,
                option_a: answerOptions[0],
                option_b: answerOptions[1],
                option_c: answerOptions[2],
                option_d: answerOptions[3],
                correct_answer: correctAnswers.join(',') // Store the correct answers as a comma-separated string
            };
            ...
            document.querySelectorAll('input[name="correct_answer[]"]').forEach(input => input.checked = false); // Clear selected checkboxes

            console.log(questions); // Log the questions array to the console
        } else {
            alert('Please fill in the question, all answer options, and select at least one correct answer.');
        }
    });
    ...
    document.querySelector('.previous-button').addEventListener('click', function() {
        if (currentQuestionIndex > 0) {
            ...
            // Check the correct answer checkboxes
            const correctAnswers = previousQuestion.correct_answer.split(',');
            document.querySelectorAll('input[name="correct_answer[]"]').forEach(input => {
                input.checked = correctAnswers.includes(input.value);
            });
        } else {
...
document.querySelector('.clear-button').addEventListener('click', function() {
    // Clear the selection
    document.querySelectorAll('input[name="correct_answer[]"]:checked').forEach(radio => {
        radio.checked = false;
    });
});
    </script>
